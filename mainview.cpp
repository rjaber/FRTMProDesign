...
MainView::MainView(const QGLFormat format, const QString &mFilePath, QWidget *parent) :
    ...
    //Calculates minimum dimensions of window based on monitor's dimensions
    const QRect SCREEN_GEOMETRY = QApplication::desktop()->screenGeometry();
    const float MIN_WIDTH_FACTOR = 0.25f;
    const float MIN_HEIGHT_FACTOR = 0.25f;
    const int MAINVIEW_MIN_WIDTH = SCREEN_GEOMETRY.width() * MIN_WIDTH_FACTOR;
    const int MAINVIEW_MIN_HEIGHT = SCREEN_GEOMETRY.height() * MIN_HEIGHT_FACTOR;

    setMinimumSize(MAINVIEW_MIN_WIDTH, MAINVIEW_MIN_HEIGHT);
    ...
    shaderProgram_.use();
        ...
        if (cameraOn_) {
            ...
            for (auto itrCamera = cameras_->cbegin(); itrCamera != cameras_->cend(); ++itrCamera) {
                projectorMatrix[cameraCount] = GLUtilities::coordSpaceToTextureSamplingAdapter *
            ...
            if (camera180FOVView_) {
                ...
                projectorPotentialMatrix = GLUtilities::coordSpaceToTextureSamplingAdapter *
