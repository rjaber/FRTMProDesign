...
bool ObjLoader::load(const QString &objFilePath, QVector<Mesh> &meshes,
    ...
    if (!objFile.open(QIODevice::ReadOnly))
        FRTMUtils::readFileErrorInfo(objFile);
    ...
    while (!iStream.atEnd()) {
        ...
        if (prefix == "v") {
            ...
            readMaterialLibrary(FRTMUtils::getModelFilePath(objFilePath) + currLineStringList[1], materials);
        }
...
bool ObjLoader::readMaterialLibrary(const QString &mtlFilePath, QVector<Material> &materials) {
    ...
    if (!mtlFile.open(QIODevice::ReadOnly))
        FRTMUtils::readFileErrorInfo(mtlFile);
