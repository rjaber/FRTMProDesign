...
void View::mouseUpdate(const QMouseEvent *newMouseEvent) {
    if (newMouseEvent->buttons() == Qt::MiddleButton)  {  //zoom in or out
        ...
        glm::mat4 rotatorMatrix = glm::rotate(glm::radians(-(newMouseEvent->x() - oldMousePosition_.x) * ROTATIONAL_SPEED_), UP_) *
                            glm::rotate(glm::radians(-(newMouseEvent->y() - oldMousePosition_.y) * ROTATIONAL_SPEED_), getViewXUP());
