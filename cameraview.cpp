...
CameraView::CameraView(MainView *mainViewGLWidget, QWidget *parent) :
    ...
    const float MIN_WIDTH_FACTOR = 0.2f;
    const float MIN_HEIGHT_FACTOR = 0.35f;
    const int CAMVIEW_SCREEN_MIN_WIDTH = SCREEN_GEOMETRY.width() * MIN_WIDTH_FACTOR;
    const int CAMVIEW_SCREEN_MIN_HEIGHT= SCREEN_GEOMETRY.height() * MIN_HEIGHT_FACTOR;

    setMinimumSize(CAMVIEW_SCREEN_MIN_WIDTH, CAMVIEW_SCREEN_MIN_HEIGHT);
